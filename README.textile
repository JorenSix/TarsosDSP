...
TarsosDSP is in the transition between "Apache Maven":http://maven.apache.org/ and "Apache Ant":http://ant.apache.org/ and currently supports both. The instructions below detail how you can build from source. Which build system you use is upto you. I find Ant a bit simpler. When everything runs correctly you should be able to run all example applications and have the latest version of the TarsosDSP library for inclusion in your projects. Also the Javadoc documentation for the API should be available.
...
h3. TarsosDSP with Maven

To build from source, you need "Apache Maven":http://maven.apache.org/ and "git":http://git-scm.com/ installed on your system. The following commands fetch the source and build the library and example jars: 
...
An implementation of the Constant-Q transform by Karl Helgason for the <a href="http://rasmusdsp.sourceforge.ne">RasmusDSP</a> project has been adapted for use in TarsosDSP. References: 
...
Judith C. Brown and Miller S. Puckette, "An efficient algorithm for the calculation of a constant Q transform":http://www.wellesley.edu/Physics/brown/pubs/effalgV92P2698-P2701.pdf,  J. Acoust. Soc. Am.,Vol. 92, No. 5, November 1992
