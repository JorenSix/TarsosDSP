...
** "Audio Time Stretching - Implementation in Pure Java Using WSOLA":http://tarsos.0110.be/artikels/lees/Audio_Time_Stretching_-_Implementation_in_Pure_Java_Using_WSOLA(Waveform Similarity Overlap Add) an implementation of the a time stretching algorithm. By using WSOLA(Waveform Similarity Overlap Add) it is possilbe to change the playback speed while audio is playing.
