 ...
 * @author Joren Six
 */
public final class NewAudioDispatcher implements Runnable {

	/**
	 * Log messages.
	 */
	private static final Logger LOG = Logger.getLogger(NewAudioDispatcher.class.getName());
	 ...
	 */
	private final List<NewAudioProcessor> audioProcessors;
	 ...
	 */
	public NewAudioDispatcher(final AudioInputStream stream, final int audioBufferSize, final int bufferOverlap)
			throws UnsupportedAudioFileException {
		
		audioProcessors = new ArrayList<NewAudioProcessor>();
		audioInputStream = stream;

		format = audioInputStream.getFormat();
		audioEvent = new AudioEvent(format, audioBufferSize, bufferOverlap);
	...
	public void setStepSizeAndOverlap(final int audioBufferSize, final int bufferOverlap){
		...
		audioEvent.setByteBuffer(audioByteBuffer);
	 ...
	 */
	public void addAudioProcessor(final NewAudioProcessor audioProcessor) {
	 ...
	 *            The AudioProcessor to add.
	 */
	public void removeAudioProcessor(final NewAudioProcessor audioProcessor) {
	...
	public void run() {
		try {
			...
			if (bytesRead != -1 && !stopped) {
				...
				for (final NewAudioProcessor processor : audioProcessors) {
	 ...
	 */
	public static NewAudioDispatcher fromFile(final File audioFile, final int size,final int overlap)
			throws UnsupportedAudioFileException, IOException {
		final AudioInputStream stream = AudioSystem.getAudioInputStream(audioFile);
		return new NewAudioDispatcher(stream, size, overlap);
	}
	 ...
	 */
	public static NewAudioDispatcher fromByteArray(final byte[] byteArray, final AudioFormat audioFormat,
		...
		return new NewAudioDispatcher(stream, audioBufferSize, bufferOverlap);
	}
	 ...
	 *             If the audio format is not supported.
	 */
	public static NewAudioDispatcher fromFloatArray(final float[] floatArray, final int sampleRate, final int audioBufferSize, final int bufferOverlap) throws UnsupportedAudioFileException {
		...
		return NewAudioDispatcher.fromByteArray(byteArray, audioFormat, audioBufferSize, bufferOverlap);
	}
}
